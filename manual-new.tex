...
\author{
	Sim\'on \'Angel\\[5pt]
	\texttt{astroaangela@gmail.com}\\[5pt]
	Paul~Eigenthaler\\
	...
	\texttt{maren551@gmail.com}\\[5pt]
     R\'egis~Lachaume\\
	\texttt{regis.lechaume@gmail.com.cl}\\[5pt]
...
The operational scheme at the La Silla 2.2~metres telescope involves minimal support by ESO.  This guide intends to familiarize observers with the operations. As a visiting astronomer please talk to the Support astronomer on site about which tasks you are expected to perform ( e.g. observing) and which will be handled by the support (e.g. troubleshooting involving servers in the dome, other electronics). The exact list of duties depends somewhat on your previous experience with the 2p2.


\section{Instrument overview}

Currently the 2p2 telescope hosts 3 instruments: \gls{wfi}, \gls{grond} and the primary workhorse \gls{feros}. During the night observations can be switched between these instruments in a matter of minutes. As most observing is done in service mode this will likely not affect you, the support astronomer will build the observing sequence, taking the programs schedule, priority and observing conditions into account. In case of visitor mode, of course the visitor program has priority, and will only  have to give time to monitoring programs. However, you should take great care when you specify said observing conditions. E.g. requesting photometric conditions and dark time for a \gls{feros} program might end you up with no observations.  Accepted programs the have to submit their OBs prior to the scheduled observing nights and for service mode also delegate the observing program to the support team. 

\subsection{WFI}

\gls{wfi} is 68-mega pixel camera, mounted on the Cassegrain focus of the 2p2 telescope (https://www.eso.org/sci/facilities/lasilla/instruments/wfi/overview.html). It provides a \ang{;34;}.0 by \ang{;33;}.0 field of view with a \ang{;;0}.238/pixel resolution. It consists of 8 CCDs, arranged in a 4x2 (x,y) pattern. However, currently there are 2 CCDs that, due to mall functioning readout electronic, can't be used.  The footprint of the \gls{wfi} is shown in the p2 tool, when you request the Finding Charts. \gls{wfi} can use a wide selection of filters, broad -band, medium-band and narrow band filters, respectively. Their characteristics can be found on  https://www.eso.org/sci/facilities/lasilla/instruments/wfi/inst/filters.html. Given the large number of filters, keep in mind that changing filters is time consuming, and adds significantly to the overhead. If you have an successful \gls{wfi} program, don't forget to add you calibration needs, e.g.  dome or sky flats in the requested filters. \gls{wfi}  calibrations do not require  Dark frames, given that the dark current is negligible.


\subsection{FEROS}

FEROS is a fibre-fed optical echelle spectrograph. It is bench-mounted and environmentally controlled.
Important instrument facts are listed below and can be found on the ESO website (https://www.eso.org/sci/facilities/lasilla/instruments/feros.html). The detailed description of the FEROS spectrograph can be found in the FEROS manual on the ESO website. The main characteristics are listed in \figureref{feroschar}.

 
\begin{figure}[!ht] 
\centering
\includegraphics[width=0.5\linewidth]{feroscharacter.jpg}
\caption[FEROS characteristics]{Main characteristics of the \gls{feros} instrument.}
\label{fig:feroschar} 
\end{figure}

During the afternoon the SA will run a set of standard calibrations (bias, flat fields, wavelength calibrations using the ThAr -lamp). In case you require a special setup for calibrations (e.g. binning of FEROS data) you need to inform the SA, or add  a calibration OB to your observing program, as those require a different set of calibrations.  The FEROS on-site pipeline will reduce the FEROS data within minutes of the observations, after they have been sent to ESO for a first quality control. Although the MIDAS pipeline provides a first impression of the data and will for instance allow the determination of the SNR to consider additional exposure time, but the results are not considered science grade. Also, the pipeline is no longer supported by ESO, i.e. there are no adjustments in the data reduction with respect to changes in the calibration lamps. As the2p2 is under MPIA management the SA can not modify the pipeline.

\subsection{GROND}

GROND is a multi-wavelength imager allowing for simultaneous imaging in 7 filters, 4 optical (g,r,i,z) and 3 near-infrared (J,H, and K) bands.  The optical field of view is \ang{;5;}.0 by \ang{;5;}.0, with a pixel resolution of \ang{;;0}.158/pixel, whereas the NIR field of view measures \ang{;10;}.0 by \ang{;10;}.0 with a pixel resolution of \ang{;;0}.60/ pixel. The design of the instrument results in a limited number of exposure time configurations, aka observing templates. They are listed and described on https://www.mpe.mpg.de/~jcg/GROND/operations.html . In contrast to \gls{wfi} and \gls{feros} at present the voT tool does not support \gls{grond} OBs written in p2. To observe with \gls{grond} still requires the OB in the old *.obd format.  Most programs require  the use of the AG for \gls{grond} observations, since the optical exposures are longer. Keep in mind that the  \gls{grond} AG sits $\sim$\ang{;28;}.0 south of the science CCDs. When you prepare your OBs it is worth checking whether the AG has suitable guide stars.



\section{Control room}

First of all, the \gls{control room}, is located below the main ESO building, across the parking lot.  It is shared with two ESO telescopes, the NTT and the 3.6\,m. If everything works without technical issue, you can do everything from there without any need to access the telescope building. In case trouble shooting includes a trip to the telescope, first contact your support astronomer, you are strongly discouraged from  power cycling the instrument servers in the telescope building or move the the telescope without prior instructions.
...
\begin{figure}[!ht] \includegraphics[width=\linewidth]{salacontrol1a.jpg}
\caption[Layout of the telescope controls]{Layout of the telescope controls.  From right to left: \gls{windows} 
controlling and monitoring the \gls{dome}, \acrshort{tcs}, \acrshort{wfi} controls,
\acrshort{feros} controls (bottom), \acrshort{grond} controls, the  \gls{p2} and \gls{vot} machine where
\gls{ob}s are crafted and loaded by the visiting astronomer, and,
finally, the FEROS \acrlong{drs}. Between the \gls{grond} and \gls{feros} workstations you find the AG screen serving both instruments. }
\label{fig:saladecontrol} 
\end{figure}

\section{Overview of operations}

In most nights, a support astronomer is present in the afternoon and the first part of the night, freeing the \gls{visitor} of the afternoon and evening twilight calibration duties.  Sometimes visitors are ask to perform all duties alone.  In that case, an experienced observer can start operations before sunset to minimize working time, while giving a small time buffer to solve the commonest technical issues.  Inexperienced ones should plan to start earlier or accept possible downtime in the very beginning of the night. Keep in mind that support by OHB is officially only guaranteed until 7 pm, unless you encounter a problem that puts the telescope in jeopardy (e.g. dome or main mirror cover can't be closed). \figureref{timeflow} shows a time-optimized chart of operations in the afternoon and both twilights, but feel free to modify according to your experience with the 2p2 and the availability of a support astronomer.

\begin{figure}[!ht]
\includegraphics[width=\linewidth]{operations-time-flow.pdf}
\caption[Time flow of afternoon and twilight observations]{Time flow of operations in the afternoon and twilight. Afternoon setup and calibrations should be started at least 2.5 hours before sunset to leave a small buffer for technical issues. This example is giving for a late fall night (May 15th), with presence required for about 15~hours.}
\label{fig:timeflow}
\end{figure}

Support astronomers perform observations in \gls{service} mode or help   the visiting astronomer  with observations in \gls{visitor} mode. 10\% of time are now  assigned to MPE observations (mostly using \gls{grond}. Other guaranteed programs may be scheduled for specific observing periods. The support astronomer will  let visiting observers know  if  one of those has to be executed during the observing run. They ensure that \glspl{visitor} correctly share their time with these other programmes.  In addition, during \gls{mpia} time, they also have to monitor the different MPIA programmes and ensure they are observed according to their priority and weather conditions. It includes real-time assessment and decisions, guided by the instructions of the different PIs, in particular during service mode.  During \gls{mpe} and Chilean time, they are mostly supporting observations and are not expecting to take real-time decisions concerning these programmes.  If service mode for MPE or Chile is offered, it is more similar to a ``delegated visitor'' mode where instructions by the PI are expected. The previously  used Rapid Response Mode observations for \gls{grond}, designed to allow the observations of Gamma-Ray Bursts has been deactivated (see \gls{grond} panel General State RRM : OFF) and will no longer interfere with observations. The \gls{grond} remote laptop has been removed.
...
\item If you walk, take the incoming traffic side and be very careful at night ( don't go out without a torch). Drivers don't use lights.
\end{itemize}
...
\item If the meteo monitor shows that the closing limits are reached (wind, humidity, clouds), close. Keep the special location of the 2p2 in mind, the humidity might be considerably higher at the 2p2 than at the other telescopes. Check the 2p2 meteo monitor\\ (\texttt{https://www.ls.eso.org/lasilla/sciops/2p2/EnvMon/2p2/})
...
In the \gls{dome} itself, many electronic boxes attached at the telescope (see \figref{dome}) may need a \gls{power cycle}, that is being switched off and on.  It is also possible to locally take control of telescope, dome, and mirror motion. If during the various startup procedures you have problems with remote controlling the telescope or the \gls{dome}, you may have to check in the dome building whether those are set to local control.
...
\caption[Entering the computer room in the telescope building]{Entering the \gls{computer room} in the telescope building. In front slightly to the left is the \gls{vme} rack, facing the door. To the right you find the \gls{grond} FIERA WS and in the far right corner the door to the telescope base and the \gls{grond} pumps. Visiting astronomers should have no reason to enter there. }
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.7\linewidth]{dome/Adam-powersupply.jpg}
\caption[Adam power supply]{At the back of ADAM you find the power supply, in case you need to powercycle. That will reset the remote/local setting for the hydraulics/drives.}
\label{fig:adam-power}
...
Start-up should take about one hour, but small technical issues can lengthen it significantly.  Ideally, it should be done early enough in the afternoon (2--3 pm in Winter, 3--4 pm in Summer), e.g. so that \gls{dome flats} and \gls{feros} calibrations can be done  before the start of the evening twilight. Since March 2025 the \gls{wfi} cooling circuit is closed and LN2 refills are no longer required. before the nitrogen of \gls{wfi} is refilled.  Also, complicated issues should be diagnosed early, so that you benefit from the day-time support from ESO engineers.  Night time support is not provided except for emergencies.
        ...
        \begin{enumerate}
          ...
          \item Wait for the filtre wheel to be set to a specific filtre (1--2 min)\label{list:wfisu-filtre}
                \begin{enumerate}
                    ...
                    \item If it stays in \texttt{MOVING} for more than 2 minutes or you skipped full restart, reset filter wheel (\procref{filtrereset}). 
                \end{enumerate}
          \item In \gls{bob}, execute \texttt{testOB.obd} (1 min)
             \label{list:wfitest}
             \begin{enumerate}
               \item Import it with \fetchob. You find it listed on the right side of the Import pop-up window.
               \item Click the \texttt{Start} button. 
             \end{enumerate}
           \item Error messages on 
           \begin{enumerate}
           	\item  If during osf2p2StartUp script appears an error message with: "... tcs header not written ..." appears in the CSS panel, run ~/bin/tcsRecoverKeyword.csh (on WFI computer), after a few exposures the error should disappear.
           	
           \end{enumerate}
           \item Failure to set filter wheel to BB{\#}Rc: PRESS RESET in Auxiliary panel on the TCS computer
          \item Bonus feature: To check the status of the WFI Pulpo:
              \begin{enumerate}
         	    \item In a terminal (w2p2ins) : cd ~/bin 
         	    \item Run: CheckPulpo.sh , this will show you the status, e.g. temperatures, alarms (Enabled) of WFI . The Telemetry panel on the TCS will also show those values, but sometimes the TCS telemetry fails and  this command helps to keep track of the status of the instrument (see Figure \figref{wfi_pulpo_status}).
         	    \end{enumerate}  
        \end{enumerate}
\bigskip
%%%%
        ...
        \item\label{lab:hydron} Switch hydraulics and drives on
             \begin{enumerate}
               ...
               \item Find the \gls{domefunc} (tab called \texttt{ADAM 6000…})\\
                     If it is not there, open it with the bookmark \texttt{Dome controls}, remember ADAM needs to be launched from Windows Explorer with the IP: 134.171.103.234 .\\
             ...
             \end{enumerate}
             It may be needed to click two or three times to get green buttons.  When you check the hydraulic and drives, have a look at the left most function \texttt{Loc/Re}, this indicates if the telescope is set to {\it{Loc}} or {\it{Re}} from inside the telescope building (most likely OHB worked on the telescope and forgot to set it back to remote). In that case you have to go to the telescope and set it back manually. Otherwise you will not be able to control the telescope from the NOB. 
             \label{list:hydr}
          ...
          \begin{enumerate}
                ...
                \item Select \wmenu{View}{Autoscale} (Optional, you can also scale manually.)
          ...
          \begin{enumerate}
            \item Check that the sidereal time is correct.\\
                  Compare the \gls{TCS control panel} and the wall clock. If the wall clock has been removed, there is a Smartphone App called 'My Siderial Time' that can be used on your phone.
            ...
            \item\label{list:pmcheck:setup} Check the \gls{pointing model} parameters are non zero
               ...
               \end{enumerate} 
            \item If they are zero or just wrong for your selected instrument, fix the \gls{pointing model}.
                \begin{enumerate}
                  \item Locate or open a tcs terminal 
                  \item Type \texttt{\home/bin/fixPointing.sh} and wait for command to return (15 s)
                  \item The script will list the \gls{pointing model} parameters for each instrument as they are currently used. Compare with the parameters displayed in the \gls{TCS setup panel}.
                  \item Redo steps \ref{list:pmcheck:wfi}--\ref{list:pmcheck:setup}
                \end{enumerate}
             \item Even if you find the pointing model to be correct, it is a safety feature to re-run the fixPointing.sh script and  re-select an instrument (most likely \gls{wfi} in preperation for twilight flats). However, it always pays to check the first pointing of the night  with a Finding Chart.
          \end{enumerate}
         \item How to handle error messages  for \gls{wfi} after \gls{tcs} restart 
         \begin{enumerate}
         	\item If the \gls{wfi} window \texttt{CSS ALARM DISPLAY} (\figref{wfialarm}) shows a {\bf{FATAL}} error that the TCS is not connected- this is the result of the initial restart of the  \gls{tcs}. As soon as the TCS is back online that error message will disappear. If the General State panel shows TCS OFF (in red), just refresh the Instrument Status via the Std. Operations menu point in the \gls{wfi} General State panel.
  ...
  \item \textbf{\gls{feros} restart (daily, 8--9 min, see \figref{feros-restart}).}\\
        ...
        \item Perform the instrument start-up (4 min) 
         ...
         \item Perform a test \gls{ob} with telescope communication (1--2 min)
           ...
           \end{enumerate}
          \item Depending on your preference you can now also start the Daily FEROS calibrations. The standard calibration runs for about 5000~sec, and has to be finished before the start of the night. Keep in mind, we still run the long term test of the Fabry-Perot, so if you plan to do that, that will add another $\approx$ 45~min. When you run calibrations OBs, deactivate the \gls{tcs}.Since the calibrations take a while there is always a chance that the TCS ha a problem (e.g. AG failure), in which case the calibration OB will crash, if the \gls{tcs} is not deactivated.
        \item \label{lab:pulpo} Check FEROS telemetry from the command line
         \begin{enumerate}
         	\item Similar to WFI, you can also check the FEROS telemetry not only via the telemetry panel on the TCS, but also via the command line on the FEROS computer.
         	\item Open terminal and login to wfefcd with: rlogin -l fcdrun wfefcd 
         	\item Run: ~/bin/fcdCheckPulpo.bsh  and check that set-points for temperatures and pressure are reached.
        \end{enumerate}
        ...
        \begin{enumerate}
         ...
         \item Do some preventive reinitialisations (1.5 min)\label{list:grond-prophylaxis}
           \begin{enumerate}
             ...
             \item In the same terminal, execute \texttt{\gls{grondSHUTTER} \&\& \gls{grondFM}} (might take 1 min). As of 2025 it might be better to run grondSHUTTER seperately, it resets the shutter of the optical camera. This will take a while. If the grondSHUTTER command finishes almost immediately, re-run the script, it should at least show 1-2 cycles of RESETing the shutter.
           \end{enumerate}
         ...
         \item Repeat the last step using communication with the TCS (2 min)\label{list:grond-testtcs}
           \begin{enumerate}
             ...
             \begin{enumerate}
                ...
                \item If error persists, close \gls{bob} and open it again. If that doesn't help, run grinsStop and grinsStart follwed by the full GROND Startup.
             \end{enumerate} 
...
\vskip 2ex
  \item \textbf{\gls{feros} and \gls{grond} guide cameras setup} (3--8 min)\label{list:ag}
        \begin{enumerate}
           ...
           \item Use numbers 6, 3, 0.02 below \texttt{Autoguider control} or {\bf{Correction time, Loop time, exposure time}} and click \texttt{Apply}.
           ...
           \item Check the \gls{feros} autoguider (1--4 min)
               \begin{enumerate}
                  ...
                  \item A bias image should be seen within seconds on the RTD display. {\bf{Flip}} the image by pressing {$\leftrightarrows$}
                  \item If it fails in the previous steps, go to \procref{restartfag} (3 min)
                  \item Click \texttt{Set Reference}
                  \item On emerging window \gls{pickrefstar}, click \texttt{Pick cursor}. After that use the mouse to click on the pixel position of {\bf{(x;y: 824.5; 515.0)}} on the RTD image. That should display a white box on the RTD, this is where during observations the FEROS Guide Star aka the target has to go. Keep in mind, the FEROS obj-fibre is fixed, so in order to guide the telescope moves to keep the guide/target star on the fiber/guide box. In contrast, both \gls{wfi} and \gls{grond}  guide by keeping the guide box on a pre-selected guide star.
               \end{enumerate}
        \end{enumerate}   
\vskip 2ex        
  \item\textbf{FEROS data reduction software setup} (2--5 min)
    \begin{enumerate}
		...
		\begin{enumerate}
		  ...
		  \item \texttt{Start} the \texttt{Midas Session Status}, it will only be finished when the MIDAS terminal (blue) ends on the command line. If you continue with the next step too early you will run 2 pipelines parallel, which block the pipeline completely. {\bf{DON'T !}}
...
\subfigure[Choose DAILY startup.]{
...
\caption[Wide field imager restart]{\gls{wfi} restart.  When done, don't forget to take a test bias. This will check whether or not the WFI computer is talking to the \gls{wfi} instrument.}
...
\subfigure[Check elements then click \texttt{OK} or simply \texttt{Skipped} to incoming pop-up.]{%
...
\begin{figure}[b!]
\null\hfill
\subfigure[Restart \gls{feros} from \texttt{wferos} terminal. Ensure that telescope is enabled in the FEROS control window.]{%
...
\null\hfill
\subfigure[Click \texttt{START}. This panel will open a new one]{
...
obtain this status.  If status is \texttt{Fail}, the camera must be restarted (see  \secref{agfail}). While you starting the FEROS AG there is one thing you can do right away- set the fiber position (Section 6.2.1 Point D4), but keep in mind that the actual fiber position might have changed. It is written  just above the AG screen.
...
screen with a line in red when the \gls{tcs} is restarted.  Get over it. }
\label{fig:wfialarm}
\end{figure}
...
\vskip 2ex
    ...
    For instance: \texttt{DomeFlatsP103.obd} or \texttt{DomeFlatsVisitorName.obd}
           ...
           \item  Frequently we have dome and skyflat OBs for specific \gls{wfi} programs, you will find them in the corresponding sub-directories in the OBs/Visitor -directory 
        \end{itemize}
        \item If choosing filtres on the fly, customise the \gls{ob}
              \begin{enumerate}
...
A linearity check is left running in the morning (see Sect.~\ref{sec:clocal}). The standard \gls{feros} calibration is generally done after startup.  It is internal in a separate room, so it can be done while doing telescope movements, going to the \gls{dome}, or even \gls{wfi} or \gls{grond} observations.
...
\begin{enumerate}
  ...
  \item Execute \texttt{StanCalNorm.obd} from \gls{bob} ($\approx 1$\,h). You may have to adjust the cal-OB, if you plan on executing different types of observations during the night, e.g. different binning, different calibration lamp.
    \begin{enumerate}
       \item Use \fetchob.
       \item Click \texttt{Start}.
    \end{enumerate}
  \item Check that the \gls{drs} has processed it. While the Fabry-Pérot is installed, the DRS will not know how to handle the first 6 lamp exposures. The DRS will still work, but the rms of the wavelength calibration might not be as good as for lamp 3 alone. 
    \begin{enumerate}
    ...
    \end{enumerate}
   \item If the DRS has issues tracing the spectral orders and is asking for interactively adjusting the threshold (in the blue MIDAS terminal) you can do that by: - lowering the current threshold (in case of NOT ENOUGH orders), or - increasing the threshold, if too many orders are detected.   
\end{enumerate}
...
\begin{enumerate}
   \item Activate the connection between instruments and telescopes
       \begin{enumerate}
         ...
         \item On the \gls{grond} control panel (\figref{grondcon}), click \texttt{TCS ON}. It should be noticed,  after the \gls{grond} Startup it is preferable to leave \texttt{TCS ON}, otherwise you may have again connection problems.
    ...
    \label{list:opencov}
     \begin{enumerate}
        \item In the \gls{auxfunc} (\figref{tcsauxfunc}), open the 
              \gls{wfi} \gls{protective shutter}, if you closed it after the dome flats.
...
Observations with \gls{feros} are managed via the OT, which is accessible via the p2-tool, or directly on the OT computer.  For \gls{wfi} you can also use both, just remember to select WFI on the OT panel. There is one minor issue, though. If you call a  \gls{wfi} OB via the OT you need to manually change the DP. TYPE keyword from inside \gls{bob}. It has to be set to OBJECT, not SCIENCE. This is a bug in the p2 for \gls{wfi}. IT is aware of his but hasn't been able to fix it. The 3rd  methods for loading \gls{wfi} OBs is directly via bob, using the old *.obd format. Those do not have the wrong keyword and can be executed directly. In contrast \gls{grond} OBs are NOT accessible via p2, and can only be loaded via bob. The RRP mode for \gls{grond} has been deactivated permanently. OBs will be sent  by Arne Rau or the corresponding PI. They can usually be found in the Visitors sub-directory.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        Done daily if possible. With flats beeing done in a single filtre at a time, WFI sjyflats can also be taken in the morning, starting at $-10$ degrees.
  ...
  \begin{enumerate}
    ...
    \begin{enumerate}
      ...
      \item Back on window \gls{ferosag}, click on fibre position.\\
            Coordinates are x=824.5 and y=515, zooming to 3 or 4$\times$ helps to click accurately.  A box centred on the pixel should appear. Attention: the fibre position might change after work on the FEROS instrument adapter at the telescope has been done. The current position values are written above the AG screen.\\
...
\begin{enumerate}
    ...
    \item When the \gls{ob} is finished, set the reference back to fibre
          (see point~\ref{list:setfibre} of \procref{switchferos}). Check the fibre position, you may have to reset it to \texttt{824.5; 515} (see \ref{list:setfibre}). 
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            \item If \gls{feros} \gls{adc} is used, it may help to change the WFI filter from $R_c$ to Cousins $I$. 
...
You can also start guiding with FEROS (it will be stable for a a short while), set up the \gls{wfi} AG, and as soon as it is active you stop the \gls{feros} AG. Otherwise both guider systems will compete with each other and reduce guiding accuracy.
...
\item Stop the exposure of the \gls{feros}/\gls{grond} AG 
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Run \gls{grond} calibrations.\\
        ...
        (\texttt{.../TEMPLATES/OBD/}). In case you have \gls{grond} OBs in both slow and fast readout mode, there is now an OB with calibrations in both modes available. 
\end{enumerate}
...
\subsection{Standard way}
 For \gls{feros} and \gls{wfi} OBs are loaded via the p2 tool.
...
\begin{itemize}
    ...
    \item For service mode observations, the PI of the program should have delegated the run to the observer, so he/she will have the same access to the \gls{ob} as the PI.
    \item To load an \gls{ob} use the Execution sequence option in \gls{p2} and \texttt{ADD} the OB to the execution sequence.
\end{itemize}


\subsection{Using ftp }
 With the new internet access settings it is no longer possible to scp OBs directly from your laptop to the corresponding computer (\gls{wfi} , \gls{feros} or \gls{grond}) as decribed in the previous section. Although that might change in the future , for now there is only one way- via ftp.
 \begin{itemize}
 	\item From your laptop (Linux)  and the directory with the files (OBs) you wish to transfer connect to the ESO/La Silla FTP -server with: > ftp -i ftp.ls.eso.org. The user will be {\bf{anonymous}}, when ask for the password, just click ENTER
 	\item Go to the directory {\bf{incoming}}, and there create a subdirectory for yourself. 
 	\item use: > put filename.obd  to copy the filename.obd into this directory.
 	\item On the \gls{wfi},\gls{feros}, \gls{grond} computer find the directory for OBs and connect to the ftp-server with ftp -i ftp.ls.eso.org
 	\item Go to the directory you created in the incoming- directory where you copied the files to and run: > get filename.* to copy the OBs from the server to the local computer.
 	\item Once the the OBs are on the local computer you can execute them  via BOB.
 \end{itemize}
 
 Keep in mind, if you are using WINDOWS or an APPLE laptop the exact ftp commands may differ. Also, \gls{feros} is now fully supported by the p2/vOT setup, so OBs for \gls{feros} can be managed via the p2 tool on your laptop, and then called up via the vOT (see next section). However,  \gls{grond} only works via the ftp procedure.
...
It is possible to go on observing with GROND and FEROS while restarting the autoguider (\acrlong{tccd}), provided that the  failing modules are deactivated on the \gls{tcs}. However, once fixed you need to reactivate them to do the last step of restarting the autoguider (\texttt{e2p2StartTCCDs}). Keep in mind, this only means that if the \gls{grond} AG failed you can continue observing with \gls{feros}/\gls{wfi} and vice versa. 
...
Lately, the \gls{grond} AG switches randomly from \texttt{ONLINE} to \texttt{STANDBY}, which shows up in both the \texttt{Autoguider} panel as well as the \gls{tcs} Status panel. Unless you want to continue with either \gls{feros} or \gls{wfi}, you need to fix the \gls{grond} AG. 
...
\begin{enumerate}
	...
	\item\label{check}
	A first check is to see whether the TCS communicates with the WFI AG. 
	\begin{enumerate}
		\item in a \gls{tcs} terminal: nslookup l2p2cam to get the IP address of the \gls{wfi} AG
		\item > ping l2p2cam or ping the IP address, if that works TCS and WFI AG are communicating- GOOD
		\item If the \gls{tcs} startup has a problem with the WFI AG, but everything else works, check whether the WFI FIERA is still online. If not- in a terminal run fcdCtrl and check if you can restart the FIERA from there (see Section 10.8.2 )
		\ once you have FIERA back ONLINE go to a \gls{tcs} terminal and run: e2p2NewStartAg Ws, this is faster than doing a full \gls{tcs} startup.\\
	\end{enumerate} 
  ...
  \item If an autoguider process is suspended, restart the \gls{tcs}
    \begin{enumerate}
       \item In a terminal, type: ps -ef, that will list all active processes. Try and find any that refer to the autoguider. You can then cancel those processes with killall -9 and the corresponding process ID
...
\subsection{Filter change not finishing}
\label{sec:filtertimeout}

Recently (09/2025) during \gls{wfi} startup or at the start of an OB the filter wheel gets stuck in {\bf{Moving}} and eventually the OB crashes with a time out error. Alternatively , even if an OB doesn't require a filter change the OB will initiate one and the filter wheel status goes back to {\bf{Moving}}.  You can try and repeat the \gls{wfi} startup, but if that doesn't solve the problem, the reason might be that the connection to the filter wheel is unstable, i.e. the ethernet cable connection to the filter wheel needs to be checked. The ethernet box can be found at the telescope, on the north side, above the filter wheel
...
\section{FEROS high noise level}

So far only once, during the FEROS data show extremely high noise levels visible in both calibration and science frames ( \figref{ferosnoise0}),  reaching up to 700 counts, whereas the 'normal' bias level should be at $\approx$240 counts. This will obviously affect the data reduction, in particular  for faint features. It will also show up in the linearity test run at the end of the night ( \figref{feroslinearity}). The first thing to check are the temperature and the pressure of the \gls{feros} instrument, e.g. in the Telemetry panel on the TCS screen, or using the FEROS checkPulpo.csh script (see Startup procedure). If those turn out to be within accepted range a possible fix is to power cycle the FEROS FIERA (\figref{feros_fiera}). 
...
\begin{enumerate} 
    ...
    \item In a terminal you will need to type some of the following
        \begin{enumerate}
            ...
            \item (needed?) \texttt{startRBS pipeline.config}; IF that never stops kill with CTRL+C
            \item \texttt{doControl stop}
            \item \texttt{doControl start wfi,feros} in a different terminal
        \end{enumerate}
\end{enumerate}
...
If there is an error about process \texttt{feoControl} stating ``accepted PING but did not reply properly within 10000 msec''', there are 2  way of fixing that. you need to restart \gls{environment} using \procref{restartenv} and go on with startup.

\procedure{Restart Startup OB}
\label{proc:restart}
The fastest way is to simply abort the Start-up OB (CTRL+C) on the \gls{feros}  terminal where you run > osf2p2StartUp ini FEROS F8, and try  the Stat-up again. If that doesn't work, try  \procref{restartenv}.
...
\paragraph{Status of FEROS lamps and/or FEROS M3 is not changing}
 Less frequently and even less critical: we had occations that the \gls{feros} General State panel did not update the status of the \gls{feros} M3 mirror ot the calibration lamp, although the ICS showed  that either had been switched on/off or moved. If that happens:\\
 \begin{itemize}
 	\item In a terminal of FEROS run: > feinsInstallScanLinks lfeics1
 	\item > scanei
 	\item Check that lfeics1 is 'ENABLED'
 \end{itemize} 
...
\begin{figure}[t!]
\begin{minipage}{0.48\linewidth}
\subfigure[\Gls{feros} frame with high noise level.]{%
\includegraphics[width=\linewidth]{feros/ferosnoise1.jpeg}
\label{fig:ferosnoise0}
}
\end{minipage}
\hspace{0.02\linewidth}
\begin{minipage}{0.48\linewidth}
\subfigure[The linearity plot of the FEROS morning linearity test with increased noise level.]{
\includegraphics[width=\linewidth]{feros/linearitytestfail.jpeg}
\label{fig:feroslinearity}}
\end{minipage}
\caption[]{FEROS high noise and failed linearity test.}	
\label{fig:ferosnoise}
\end{figure}
...
\begin{itemize}
  ...
  \item The \gls{wfi} startup will set the filter to BB{\texttt{\#}}Rc/162{\texttt{\_ESO844}}. In case the filter wheel gets stuck in \texttt{Moving} on the \gls{wfi}, press the \texttt{Reset} button on the \gls{TCS aux panel}.
...
\end{enumerate}
\vskip 2ex
\procedure{Power cycle ADAM}
Sometimes it seems to be impossible to switch the hydraulics on/off from the NOB, i.e. it switches back after a few seconds from whichever position it was. In that case, you can try and power cycle ADAM.
\begin{enumerate}
	\item Go to the dome.
	\item Locate the gray 1.5-metre-high \gls{adam} rack (\figref{dome}).
	\item Locate the  (light blue) power supply switch for \texttt{ADAM power} at the BACK of \gls{adam} (\figref{adam-power}).
	\item Unplug the top part and after a short while plug in again. That should reset the power supply.
...
For small zenithal angles there is an apparent misalignment between \gls{slit} and telescope orientation as shown on the \gls{rose diagram} on the \gls{tcs} control panel. It is a normal setting since the telescope is not exactly at the center of the \gls{dome}.  If you need to avoid the wind you need to orient the \gls{dome} manually but be weary of vignetting.

\section{GROND issues}

\subsection{GROND startup failed with FIERA stuck in LOADING}
 In general the \gls{grond} startup is straight forward (see Section 2.1.D). But as so often with \gls{grond} every now and then the startup will either not finish ( CNTRL+C needed) or finish with FIERA in status LOADED. There are several procedures to try.
 
 \begin{enumerate}
 	\item Run grinsStop/grinsStart, followed by the Start-up procedure.
 	\item $~/bin/HowtoLastResort\_Before*.sh$ followed by  $~/bin/HowtoLastResort\_After2*.sh$. You can run both shell scripts directly, or  copy and paste the individual steps to the command line. The latter will make it easier to check if and when a step fail. After that try Start-up again.\\
 	\item The following procedure is the most time consuming and requires a trip to the telescope. Btw, this was described by Arne on June 16, via  slack, and a print out of it has been put into the \gls{grond} FIERA rack in the VME room.
 		\begin{enumerate}
 			\item If the \gls{grond} Start-up failed, make sure you are in a defined status, e.g. abort the startup procedure, but no active scripts.
 			\item Go to the server room and locate the FIERA Sparc (in the electronics rack in the VME room), check what you can see on the monitor (named wgrccd display),most likely it will be something like \figref{fiera1}. If you see a prompt, try a 'shutdown now'. If not, switch off the machine (likely at the back), wait a few seconds , and witch on again.
 			\item When wgrccd restarted, you should see something like the top lines in \figref{fiera1}.
 			\item Enter the root password (written below the screen or on the keyboard) and wait until you get a prompt.  The system may try an automatic fsck disc check. If it doesn't, and instead goes directly to the prompt, run: 'fsck -Fufs /dev/rdsk/c0t3d0s0' (0=zero) and enter 'Y' for all queries (see \figref{fiera2}).
 			\item After the fsck is completed, you should get the prompt back. Then, head up to the telescope and switch OFF/ON the FIERA electronics. This is the box with the sticker 'Never Switch OFF'. You will need a ladder to reach it. The switch is  a bit hidden (see \figref{fiera3}) at the lower part of the FIERA.  It is {\bf{IMPORTANT}} that this box is never switched OFF for an extended amount of time (> minutes) as it powers the CCD heater controls. If they are not working , the CCDs will become to cold and break. That's why it is important to do that now, and not first aka before rebooting and fcsk-ing wgrccd.
 			\item Go down to the server (VME) room and do another 'shutdown now' for wgrccd. Repeat the steps above, i.e. login as root and do an fsck.
 		\end{enumerate}
 \end{enumerate}
...
\begin{figure}[t!]
	\begin{minipage}{0.5 \linewidth}
		\subfigure[wgrccd display]{%
			\includegraphics[width=\linewidth]{grond/fiera1.jpg}
			\label{fig:fiera1}
		}
	\end{minipage}
	\hspace{0.02\linewidth}
	\begin{minipage}{0.5\linewidth}
		\subfigure[wgrccd display with fcsk updates]{
			\includegraphics[width=\linewidth]{grond/fiera2.jpg}
		  \label{fig:fiera2}
		 }
	\end{minipage}
		\hspace{0.02\linewidth}
\begin{minipage}{0.5\linewidth}
	...
	\subfigure[FIERA electronic box at the telescope (north side) with the power switch.]{
		\includegraphics[width=\linewidth]{grond/fiera3.jpg}
	     \label{fig:fiera3}
	    }
\end{minipage}	
	
	\caption[ Rebooting GROND FIERA]{Reboot FIERA and wgrccd in case of \gls{grond} Start-Up failure or FIERA failure. }
	\label{fig:fierareboot}
...
\begin{enumerate}
  ...
  \item Do \texttt{\home/bin/lastResortAfterReboot\_2.sh} as user \texttt{grondmgr}
...
\subsection{GROND data disk full- OB crashes}

GROND takes a large amount of data, mostly due to the short NIR images. As a result, chances are that the disk space, in particular in /diskb/data/DETDATA. When that happens any GROND \gls{ob} will crash. The solution, open a terminal on grondmgr and run df -hl, to see which disk is full and needs to be cleaned.  That will most likely be /diskb, where the GROND data are stored in the data -subdirectory. Unfortunatelly, the files are not named using the time stamp, but a sequencial number. One solution is to delete all data older than  a given interval, e.g. anything older than 60 dyas. The correct command line would be: 

find /diskb/data/DETDATA -type f -mtime +60 -exec rm -f {} $\backslash;$

Keep in mind, the $\backslash;$ is IMPORTANT, to escape the semicolon, which tells the shell that the semicolon is part of the find command and not a shell command separator. 
...
ID        & $-150.86$  & $-28.04$ & $-294.28$\\
IH        & $+33.80$  & $-98.00$ & $+42.83$\\
...
\begin{enumerate}   
    ...
    \item Check that the Sidereal time on the \gls{TCS control panel} is fine.\\
      ...
      If not, you can try the fast fix by running lccBoot lte2p2 in a TCS terminal. Make sure you send the telescope to the zenith first, either using the TCS Setup panel or the presetzenith command in a TCS terminal. After that, you need to initialize the telescope. When this is done check again the Siderial Time. If that didn't work, you need to go to the telescope building and reboot the \gls{vme}. After that is done, again you need to initialize the telescope.
...
\label{proc:park}
\begin{enumerate} 
	\item Ensure that the telescope hydraulics and drives are ON.    
...
It may happen that the \gls{webcam} doesn't show a live image of the dome. Sometimes that happens after the light changes drastically, e.g. after  switching on/off the flat field lamp or after opening/closing the dome during day light. In that case, reload the web-page and press the video symbol. In case  you need to reconnect to the webcam an it asks for the password: it's either AXIS or axis\\
...
\subsection{Start ADAM display after restarting the Windows computer}
If the windows computer with the webcam and the ADAM display has to be rebooted, you have to open a Explorer (!!!! different than dome webcam) window and open:\\
\url{http://134.171.103.234} . It will ask for the 'good seeing' password we use for pretty much anything .
...
If \gls{bob} does not play sounds to indicate pop-ups or observation finishing, the sound server should be restarted. In fact if the sound server is off you will not be able to start new observations.

\procedure{Restart the sound server}
\begin{enumerate}
    \item On any of the workstation, locate or open a terminal
    \item Type \texttt{ssh grondmpe@wgrondoff "pulseaudio {-}{-}kill; pulseaudio {-}{-}start"}, and Enter password (see Table~\ref{tab:pwd})
    \item Note: It seems that splitting the above command into separate steps works slightly more reliable: 
    \begin{enumerate}
    	\item \texttt{ssh grondmpe@wgrondoff} (Enter password, see Table~\ref{tab:pwd})
    	\item \texttt{"pulseaudio {-}{-}kill"}
    	\item \texttt{"pulseaudio {-}{-}start"}
    \end{enumerate}
...
\begin{figure}[!ht]
\centering
  \includegraphics[width=1\linewidth]{dome/dome-webcam.jpg}%
...
vot computer                 & \gls{visitor}                              & .5ar$\cdots$\\
dhs computer                 & pipeline                                   & .5arcs$\cdot$\\
...
Name                    &  Phone                                            & Service\\
\hline\hline
Francisco Duran	        & 5575                                              & IT Paranal \\
Pablo Cordero           & 3157                                              & IT Santiago \\
Josefina Urrutia        & 5281                                              & IT Paranal \\
Javier Alarc\'on        & 3119                                              & IT Santiago \\
 2p2 dome               & 4109                                              & 2p2 dome floor\\
 Jos\'e Orrego          & +56991297177 / +56512272601                       & ESO Reception La Serena\\
 Francisco Tapia        & 4172, +56949910962                                & La Silla Reception\\
 Claudio Espinoza       & +56976957745                                      & OHB \\
 Justo Perez            & 4100, +56934245907                                & La Silla Logistics\\
 Ruben Vega             & 4100, +56986700107                                & La Silla Logistics\\
                        & +56998873763                                      & La Silla Logistics/ hospitality\\
 \hline
\end{tabular}
\end{table}
